...
$(function(){
	...
	// 첨부파일 목록 불러오기 
	$.getJSON(`${ctxPath}/board/${boardType}/getAttachList`,{bno:bnoValue},function(attachList){
		let fileList = '';
	...
	$('.uploadResultDiv ul').on('change','.toBeDelFile',function(e){
		...
		if($(this).is(':checked')){
			$.ajax({
				type : 'get', 
				url : `${ctxPath}/board/${boardType}/getAttachFileInfo`, 
				data : {uuid : uuid}, 
	...
	$('.modifyForm button').click(function(){
		...
		addCriteria();
		if(operation=='remove'){
			formObj.attr('action',`${ctxPath}/board/${boardType}/remove`);
